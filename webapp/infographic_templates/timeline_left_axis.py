...
META = {
    ...
    "params": [
        ...
        {"key": "axis_x", "label": "时间轴位置", "type": "range", "min": 100, "max": 300, "step": 10, "default": 160},
...
def build(data: dict, params: dict) -> str:
  ...
  <text x="{axis_x - 16}" y="{y + 5}" text-anchor="end" font-family="'IBM Plex Mono',monospace" font-size="15" font-weight="700" fill="{accent}">{_esc(year)}</text>
  <text x="{axis_x + 16}" y="{y - 8}" font-family="'Noto Sans SC','PingFang SC',sans-serif" font-size="{title_size}" font-weight="700" fill="#FFFFFF">{_esc(title)}</text>
  <text x="{axis_x + 16}" y="{y + 14}" font-family="'Noto Sans SC','PingFang SC',sans-serif" font-size="13" fill="rgba(255,255,255,0.55)">{_esc(desc)}</text>""")
