...
一个本地运行的三段式生产工具：研究一家 AI 公司，人工定稿文字和图片，通过卡片编排和模板排版生成可配置数量的知识卡片。内置三套规格：v1 经典 8 张、v2 新版 7 张、v3 研究增强版 8 张，可在定稿台顶部切换。
...
在研究台输入公司名和官网，点击”开始研究”。研究完成后进入定稿台，按「卡片设置 → 文字定稿 → 图片定稿 → 进入排版」的新流程操作。定稿台顶部可切换 v1/v2/v3 套卡（卡片数量和字段结构不同），文字定稿支持从标准版/商业版/传播版一键采用并编辑；卡片设置可自由增减卡片、分配字段和图片引用。
...
默认每张卡会导出 3 张高倍率候选图（`--shots 3 --scale 3`），默认套卡为 `--set v1`；导出新版 7 张卡传 `--set v2`，导出研究增强版 8 张卡传 `--set v3`。需要更少或更高倍率可改 `--shots`、`--scale`。
