...
2. The pipeline collects Tavily, GitHub, YouTube, and website content in parallel. Tavily supports `TAVILY_API_KEYS` as a comma-separated fallback list; quota responses try the next key before marking the Tavily chain failed. Deep Tavily plans cover overview/founders/funding/product plus operating metric intents: market size, revenue metrics, user metrics, retention metrics, unit economics, and capital efficiency. Long Tavily queues report partial progress after each query so the research desk does not stay at "waiting" until the whole queue completes.
