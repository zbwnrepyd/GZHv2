...
- `webapp/infographic.py`: SVG/HTML renderers for infographics. Flywheel/timeline use LLM-extracted JSON → SVG templates → Playwright PNG. chart_competitive（竞争格局定位图）和 chart_ecosystem（AI 栈生态位图）使用 ECharts HTML：markArea 象限背景 + 中文轴名/象限标签 + 目标公司高亮（青色 1.5×气泡）+ light 主题 900×600 → Playwright 2x PNG。ecosystem Y 轴为 category 泳道（splitArea 交替背景）。内联 `webapp/static/vendor/echarts.min.js`，不依赖 CDN。CSS 禁用 vw/vh（srcdoc iframe 坍塌），使用 `position:absolute;inset:0`。
