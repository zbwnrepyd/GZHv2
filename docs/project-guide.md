...
输入公司名 + 官网 URL → 自动研究 → 人工定稿 → 输出 v1 8 张、v2 7 张或 v3 研究增强版 8 张 3:4 知识卡片 PNG。
...
2. **定稿台** `http://127.0.0.1:5050/editor?company=<公司名>&set=v1|v2|v3` — 顶部切换套卡→卡片设置→文字定稿→图片定稿，左栏底部固定「进入排版」
3. **卡片制作台** `http://127.0.0.1:5050/canvas/?company=<公司名>` — HTML/CSS 卡片编辑和预览
4. **排版中心** `http://127.0.0.1:5050/layout?company=<公司名>&set=v1|v2|v3` — 选模板、调图层、改文字位置和样式
