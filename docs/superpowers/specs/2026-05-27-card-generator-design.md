...
│              │                                      │                │
│ 项目名只读   │ 顶部：当前卡片名 + 返回/适配/导出      │ HTML+CSS源码   │
│ 卡片/图片夹  │ 中部：3:4 HTML 卡片 iframe            │ 语法高亮/保存  │
│ 导出入口常驻 │ 底部：图片 prompt bar                 │ 当前页源码     │
└──────────────┴──────────────────────────────────────┴────────────────┘
```

布局要求：

- 左栏只放必要导航，避免挤占画布；公司名来自定稿台跳转或 `?company=`，不在制作台内编辑。
- “卡片每一页”和“图片夹”是互斥手风琴；打开一个会收起另一个，长内容在打开面板内滚动。
- 图片夹展示已确认 Markdown 中的图片 URL、底部 prompt bar 生成过的图片，以及背景水印上传/清除控件；导出入口不放入折叠区。
...
4. `card-renderer.html` 包含只读项目名、互斥手风琴、图片夹和常驻导出按钮。
5. JS 中存在 `localStorage` 样式状态 key。
6. JS 中存在源码高亮和实时渲染入口。
